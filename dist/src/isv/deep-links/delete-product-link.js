...
/**
 * Delete deep link for a product (ISV on behalf of merchant)
 */
export const deleteProductLinkTool = {
  name: "payware_deep_links_delete_product_link",
  description: `Delete an active deep link for a product on behalf of a merchant. Once deleted, the deep link URL will no longer redirect to the payware mobile app.
...
- This operation is irreversible`,
  ...
  inputSchema: {
    ...
    properties: {
      ...
      merchantPartnerId: {
        type: "string",
        description: "Partner ID of the merchant for whom the product is being deleted"
      },
      oauth2Token: {
        type: "string",
  ...
  async handler(args) {
    ...
    try {
      ...
      return {
        content: [{
          type: "text",
          text: `✅ **Product Deep Link Deleted (ISV)**

**Deletion Details:**
- Product ID: ${productId}
- Merchant: ${merchantPartnerId}
- Status: Successfully deleted
- Link URL: https://url.payware.eu/${productId} (now inactive)
...
⚠️ Any existing shared links will stop working immediately`
      ...
      return {
        content: [{
          type: "text",
          text: `❌ **Failed to Delete Product Deep Link (ISV)**
